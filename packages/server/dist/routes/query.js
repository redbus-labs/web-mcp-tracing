...
router.get('/apps', async (req, res) => {
    try {
        const apps = await db_1.default.toolExecution.groupBy({
            by: ['appId'],
            _count: { appId: true },
...
router.get('/summary', async (req, res) => {
    try {
        ...
        const totalExecutions = await db_1.default.toolExecution.count({ where });
        const successfulExecutions = await db_1.default.toolExecution.count({
            where: { ...where, status: 'SUCCESS' },
        });
        const errorExecutions = totalExecutions - successfulExecutions;
        const activeSessions = await db_1.default.session.count({
            where: {
                appId: where.appId,
                createdAt: where.createdAt
            }
        });
        const avgResult = await db_1.default.toolExecution.aggregate({
            where,
...
router.get('/tools', async (req, res) => {
    try {
        const where = getFilters(req);
        const toolCounts = await db_1.default.toolExecution.groupBy({
            where,
            by: ['toolName'],
        ...
        res.json(toolCounts.map((t) => ({ name: t.toolName, count: t._count.toolName })));
    }
    catch (error) {
...
router.get('/volume', async (req, res) => {
    try {
        ...
        const executions = await db_1.default.toolExecution.findMany({
            where,
            select: { createdAt: true, status: true },
            orderBy: { createdAt: 'asc' }
        });
        // Group by hour or day depending on range
        const grouped = executions.reduce((acc, curr) => {
            const dateStr = curr.createdAt.toISOString().substring(0, 13) + ':00:00Z'; // group by hour
            if (!acc[dateStr])
                acc[dateStr] = { time: dateStr, SUCCESS: 0, ERROR: 0 };
...
router.get('/executions', async (req, res) => {
    try {
        ...
        if (toolName && toolName !== 'all')
            where.toolName = String(toolName);
        if (sessionId)
        ...
        const [items, total] = await Promise.all([
            db_1.default.toolExecution.findMany({
                where,
            ...
            db_1.default.toolExecution.count({ where })
        ]);
...
router.get('/sessions', async (req, res) => {
    try {
        ...
        const where = {};
        if (baseWhere.appId)
            where.appId = baseWhere.appId;
        if (baseWhere.createdAt)
            where.createdAt = baseWhere.createdAt;
        if (search) {
            where.OR = [
                { id: { contains: String(search), mode: 'insensitive' } },
        ...
        const [items, total] = await Promise.all([
            ...
            db_1.default.session.count({ where })
        ]);
        res.json({ items, total });
    }
    catch (error) {
...
router.get('/traces', async (req, res) => {
    try {
        ...
        if (baseWhere.appId || baseWhere.createdAt) {
            where.toolExecution = {};
            if (baseWhere.appId)
                where.toolExecution.appId = baseWhere.appId;
            if (baseWhere.createdAt)
                where.toolExecution.createdAt = baseWhere.createdAt;
        }
        const [items, total] = await Promise.all([
            db_1.default.apiTrace.findMany({
                where,
                orderBy: { createdAt: 'desc' },
                skip: Number(skip),
                take: Number(take),
            ...
            db_1.default.apiTrace.count({ where })
        ]);
        res.json({ items, total });
    }
    catch (error) {
...
router.get('/recent', async (req, res) => {
    try {
        const where = getFilters(req);
        const recent = await db_1.default.toolExecution.findMany({
            where,
            take: 20,
            orderBy: { createdAt: 'desc' },
            include: {
